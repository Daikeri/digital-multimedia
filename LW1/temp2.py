...
# Функция для заполнения внутренности креста ближайшим цветом
def fill_cross_color(frame, rect_start_point, rect_end_point):
    # Определяем координаты креста
    x1, y1 = rect_start_point
    x2, y2 = rect_end_point

    # Определяем центральные координаты креста
    cross_center_x = int((x1 + x2) / 2)
    cross_center_y = int((y1 + y2) / 2)

    # Получаем текущий цвет центрального пикселя
    center_pixel_color = frame[cross_center_y, cross_center_x]

    # Заданные цвета в формате RGB
    red = np.array([0, 0, 255])
    green = np.array([0, 255, 0])
    blue = np.array([255, 0, 0])

    # Вычисляем евклидово расстояние между текущим цветом и каждым из трех цветов
    distance_to_red = np.linalg.norm(red - np.array(center_pixel_color))
    distance_to_green = np.linalg.norm(green - np.array(center_pixel_color))
    distance_to_blue = np.linalg.norm(blue - np.array(center_pixel_color))

    # Находим минимальное расстояние
    min_distance = min(distance_to_red, distance_to_green, distance_to_blue)

    # Определяем цвет, ближайший к текущему цвету
    if min_distance == distance_to_red:
        closest_color_rgb = red
    elif min_distance == distance_to_green:
        closest_color_rgb = green
    else:
        closest_color_rgb = blue

    # Заполняем внутренность креста ближайшим цветом
    frame[y1:y2, x1:x2] = closest_color_rgb
...
while True:
    ...
    # Заполняем внутренность крестов ближайшими цветами
    result_frame = fill_cross_color(result_frame, (rect_start_x_horizontal, rect_start_y_horizontal),
                                    (rect_end_x_horizontal, rect_end_y_horizontal))
    result_frame = fill_cross_color(result_frame, (rect_start_x_vertical, rect_start_y_vertical),
                                    (rect_end_x_vertical, rect_end_y_vertical))

    # Отображаем результат
    cv2.imshow('Frame with Blurred Rectangles and Colored Cross', result_frame)
